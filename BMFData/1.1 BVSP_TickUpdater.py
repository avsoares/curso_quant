...
download_path = BMFData_path + "Download/"
temp_path = BMFData_path + "Temp/"
basepath = BMFData_path
unzip_path = BMFData_path + "Temp/"
...
if download_ == 1:
    ...
    for i in range(files_raw.shape[0]):
        if "NEG_2" in files_raw[i] and not files_raw[i] in downloaded:
            print(files_raw[i])
...
if insert_ == 1:
    ...
    #for j in range(200, 323):
        last_bd_name = temp_files[j][4:10]
        ...
        try:
            temp_ = pd.read_csv("BVSP_Temp/" + zip_files[0], sep=';', decimal=',', names=Header)
        except:
            temp_ = pd.read_csv("BVSP_Temp/apphmb/intraday/" + zip_files[0], sep=';', decimal=',', names=Header)
        if temp_.shape[0] < 10:
            try:
        ...
        if 1 == 1:
            ...
            Time = np.array([datetime.strptime(data_temp[i] + " " + time_temp[i], '%Y-%m-%d %H:%M:%S.%f').replace(tzinfo=pytz.utc) for i in range(1, len(time_temp) - 1)])
