...
if insert_ == 1:
    ...
    for j in range(0, len(temp_files)):
        ...
        try:
            temp_ = pd.read_csv("Temp/" + zip_files[0], sep=';', decimal=',', names=Header)
        except:
            temp_ = pd.read_csv("Temp/apphmb/intraday/" + zip_files[0], sep=';', decimal=',', names=Header)
        if temp_.shape[0] < 10:
            try:
        ...
        if 1 == 1:
            ...
            Time = np.array([datetime.strptime(data_temp[i] + " " + time_temp[i], '%Y-%m-%d %H:%M:%S.%f').replace(tzinfo=tzlocal.get_localzone()) for i in range(1, len(time_temp) - 1)])
